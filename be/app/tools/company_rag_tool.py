...
company_rag_prompts = PROMPTS.company_rag
...
class CompanyRagTool:
    def __init__(self, vectorstore_path, embedding_function, model_name="gpt-4o-mini"):
        ...
        PROMPT_TEMPLATE = company_rag_prompts['prompt_template']
        self.prompt_template = ChatPromptTemplate.from_template(PROMPT_TEMPLATE)
    
    def retrieve_and_answer(self, query, k=5):
    ...
    def create_tool(self,):
        return Tool(
            name=company_rag_prompts['name'],
            func=lambda query: self.retrieve_and_answer(query),
            description=company_rag_prompts['description']
        )